...
import {
  LayoutDashboard, Briefcase, MessageSquare, ListTodo, Heart,
  Settings, LogOut, Search, Bell, Mail, ChevronRight,
  Star, TrendingUp, Users, Clock, Plus, ExternalLink
} from 'lucide-react';
...
const vagasRecentes = [
  ...
  {
    title: 'Redesign de Identidade Visual Completa',
    platform: '99Freelas',
    tag: 'DESIGN',
    tagColor: '#a78bfa',
    price: 'R$ 2.200',
  ...
  {
    title: 'Gestão de Tráfego Pago no Google Ads',
    platform: 'GetNinjas',
    tag: 'MARKETING',
    tagColor: '#34d399',
    price: 'R$ 1.800',
...
const plataformas = [
  { name: 'Workana', role: 'Plataforma', status: 'Conectado' },
  { name: 'GetNinjas', role: 'Plataforma', status: 'Conectado' },
  { name: '99Freelas', role: 'Plataforma', status: 'Conectado' },
];

const sidebarLinks = [
  { icon: LayoutDashboard, label: 'Dashboard', active: true },
  { icon: Briefcase, label: 'Vagas', active: false },
  { icon: MessageSquare, label: 'Mensagens', active: false },
  { icon: ListTodo, label: 'Propostas', active: false },
  { icon: Heart, label: 'Favoritos', active: false },
];

const friends = [
  { name: 'Carlos Silva', role: 'Dev Frontend' },
  { name: 'Ana Beatriz', role: 'Designer UX' },
  { name: 'João Pedro', role: 'Marketing' },
];
...
const Dashboard = () => {
  ...
  return (
    <div className="flex h-screen" style={{ background: '#F8F9FC' }}>
      {/* LEFT SIDEBAR */}
      <aside className="w-[220px] shrink-0 flex flex-col justify-between border-r border-[#E8ECF4] bg-white py-6 px-4 max-lg:hidden">
        <div>
          {/* Logo */}
          <button onClick={() => navigate('/')} className="flex items-center gap-2 mb-8 px-2 hover:opacity-80 transition-opacity">
            <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: 'hsl(var(--primary))' }}>
              <Search size={14} className="text-white" />
            </div>
            <span className="font-heading font-extrabold text-lg text-[#1A1D26]">ikas</span>
          </button>

          {/* Overview */}
          <p className="text-[11px] font-body font-medium text-[#9CA3B4] uppercase tracking-wider mb-3 px-2">Overview</p>
              ...
              <button
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-body font-medium transition-colors ${
                  link.active
                    ...
                    : 'text-[#6B7280] hover:text-[#1A1D26] hover:bg-[#F3F4F6]'
                }`}
                style={link.active ? { background: 'hsl(var(--primary) / 0.08)' } : undefined}
              >
                <link.icon size={18} />
                {link.label}
              </button>
          ...
          {/* Contatos */}
          <p className="text-[11px] font-body font-medium text-[#9CA3B4] uppercase tracking-wider mb-3 px-2">Contatos</p>
          <div className="flex flex-col gap-3 px-2">
            {friends.map((f) => (
              <div key={f.name} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#a78bfa] to-[#38bdf8] flex items-center justify-center text-white text-xs font-bold">
                  {f.name.split(' ').map(n => n[0]).join('')}
                </div>
                <div>
                  <p className="text-sm font-body font-medium text-[#1A1D26] leading-tight">{f.name}</p>
                  <p className="text-[11px] font-body text-[#9CA3B4]">{f.role}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-1">
          <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-body font-medium text-[#6B7280] hover:text-[#1A1D26] hover:bg-[#F3F4F6] transition-colors">
            <Settings size={18} /> Configurações
          </button>
          <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-body font-medium text-red-500 hover:bg-red-50 transition-colors">
            <LogOut size={18} /> Sair
          </button>
        </div>
      </aside>
        ...
        <header className="flex items-center justify-between h-16 px-6 bg-white border-b border-[#E8ECF4] shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 ml-2">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#a78bfa] to-[#f472b6] flex items-center justify-center text-white text-xs font-bold">
                JR
              </div>
              <span className="text-sm font-body font-medium text-[#1A1D26] max-md:hidden">João Ramos</span>
            </div>
          </div>
        </header>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <button className="w-8 h-8 rounded-full border border-[#E8ECF4] flex items-center justify-center text-[#9CA3B4] hover:text-[#1A1D26] transition-colors">
                      ‹
                    </button>
                    <button className="w-8 h-8 rounded-full flex items-center justify-center text-white" style={{ background: 'hsl(var(--primary))' }}>
                      ›
                    </button>
                  </div>
                </div>
                    ...
                    <div key={vaga.title} className="bg-white rounded-2xl border border-[#E8ECF4] overflow-hidden group hover:shadow-lg transition-shadow">
                      ...
                      <div className="p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-[10px] font-body font-bold uppercase tracking-wider px-2 py-0.5 rounded" style={{ color: vaga.tagColor, background: `${vaga.tagColor}15` }}>
                            {vaga.tag}
                          </span>
                    ...
                    <tbody>
                      {lessonRows.map((row, i) => (
                        <tr key={i} className="border-b border-[#E8ECF4] last:border-0">
                          ...
                          <td className="px-5 py-3">
                            <span className="text-[10px] font-body font-bold uppercase tracking-wider px-2 py-0.5 rounded" style={{ color: row.tagColor, background: `${row.tagColor}15` }}>
                              {row.tag}
                            </span>
                          </td>
            ...
            <div className="w-[300px] shrink-0 border-l border-[#E8ECF4] bg-white p-6 space-y-6 max-xl:w-full max-xl:border-l-0 max-xl:border-t max-xl:flex max-xl:gap-6 max-xl:flex-wrap">
              ...
              <div className="flex-1 min-w-[260px]">
                ...
                <div className="flex flex-col items-center mb-4">
                  ...
                  <p className="font-heading font-bold text-lg text-[#1A1D26]">Bom dia, João 🔥</p>
                  ...
                  {statsData.map((s) => (
                    <div key={s.label} className="flex-1 flex flex-col items-center gap-1">
                      <div
                        ...
                        style={{
                          height: `${s.value * 2}px`,
                          background: s.value === Math.max(...statsData.map(d => d.value))
                            ? 'hsl(var(--primary))'
                            : '#E8ECF4',
                        }}
                      />
              ...
              <div className="flex-1 min-w-[260px]">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button className="w-7 h-7 rounded-full border border-[#E8ECF4] flex items-center justify-center text-[#9CA3B4] hover:text-[#1A1D26] transition-colors">
                    <Plus size={14} />
                  </button>
                </div>
                <div className="flex flex-col gap-3">
                  {plataformas.map((p) => (
                    <div key={p.name} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#a78bfa] to-[#38bdf8] flex items-center justify-center text-white text-[10px] font-bold">
                      ...
                      <button className="text-[11px] font-body font-medium px-3 py-1 rounded-full" style={{ color: 'hsl(var(--primary))', background: 'hsl(var(--primary) / 0.08)' }}>
                        {p.status}
                      </button>
                    </div>
                  ))}
                </div>
                <button className="w-full mt-4 py-2.5 rounded-xl text-sm font-body font-medium transition-colors" style={{ background: 'hsl(var(--primary))', color: 'white' }}>
                  Ver Todas
                </button>
              </div>
            </div>
